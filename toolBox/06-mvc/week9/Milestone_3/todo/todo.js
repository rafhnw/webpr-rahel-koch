...
const TodoController = () => {
    ...
    // ist neu eine ObservableList
    const todoModel = ObservableList([]); // observable array of Todos, this state is private

    const addTodo = () => {
        const newTodo = Todo();
        todoModel.add(newTodo); // hier wird nicht mer gebusht, sonder add aufgerufen
        return newTodo;
    };
...
const TodoView = (rootElement, numberOfTasksElement, numberOfOpenTasksElement) => {
    ...
    // dinge werden auseinandergezogen, dadurch klarheit schaffen
    todoController.onTodoAdd   (renderNewRow); // rendern wenn es ein neues todo gibt
    todoController.onTodoAdd   (statsUpdate); // status update wenn neues
    todoController.onTodoRemove(statsUpdate); // statusupdate wenn remove
